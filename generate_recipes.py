...
configs = [
    {"groups": 600, "prompts": 15, "concurrency": 250},
    {"groups": 450, "prompts": 30, "concurrency": 150},
    {"groups": 150, "prompts": 30, "concurrency": 80},
    {"groups": 200, "prompts": 50, "concurrency": 100}
]

def generate_recipe(groups, prompts, concurrency, is_offload):
    mode = "offload" if is_offload else "base"
    name = f"qwen3-235b-{mode}-{groups}g-{prompts}p"
    
    # kv_transfer_config for vllm serve
...
for c in configs:
    for is_offload in [True, False]:
        name, content = generate_recipe(c['groups'], c['prompts'], c['concurrency'], is_offload)
